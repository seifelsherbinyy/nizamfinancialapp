...
## 12. A cross-repository name gap, not an in-repo defect

This repository's own environment contract is self-consistent: every "ref"-style entry in
this repository's env template (`ops/env/life.env`, committed with the conventional `.example`
suffix) resolves to a real port field through `envTemplates.ts` `CODE_BINDINGS`, and
`npm run verify:all -- --all` holds its baseline throughout this finding.
The gap sits between repositories, not inside either one.

The other repository's relay reads its configuration under different names. Verified this
session by direct read of `NIZAM__system/relay/poller.py`, `webhook.py` and `auth.py` in the
clone at the path recorded in this file's header:

| This repository's contract entry | Bound to (port field) | Other repository's actual name | Verified at |
|---|---|---|---|
| `BOT_A_TOKEN` | raw secret, no CODE_BINDING | `TELEGRAM_BOT_TOKEN` | `poller.py` line 212 |
| `TELEGRAM_MODE` | `ports/telegram.ts` `mode` | `RELAY_MODE` (`standby`\|`live`) | `poller.py` line 220 |
| `ALLOWED_USER_IDS` | `ports/telegram.ts` `allowedSenderIds` | `NIZAM_TELEGRAM_ALLOWED_IDS` | `auth.py` lines 23, 82; `poller.py` docstring line 33 |
| `LIFE_WEBHOOK_SECRET` | `ports/telegram.ts` `expectedSecretToken` | `TELEGRAM_WEBHOOK_SECRET` | `auth.py` lines 22, 51 |
| `MSG_API_BASE` | `ports/telegram.ts` `apiBaseUrlRef` | none, hardcoded to Telegram's own public API host | `poller.py` line 55 |
| no entry on this side | n/a | `NIZAM_TG_POLL_TIMEOUT` (default `25`), long-poll runner only | `poller.py` line 206 |
| no entry on this side | n/a | `TELEGRAM_WEBHOOK_PATH` (default `/telegram`), webhook runner only | `webhook.py` line 76 |
| no entry on this side | n/a | `RELAY_HOST` (defaults to the local loopback address), `RELAY_PORT` (defaults to a fixed local port), webhook runner only | `webhook.py` lines 114-115 |

Every row on this repository's side was already confirmed against its own port fields before
this finding; nothing above required touching this repository's env template, and it has not
been touched. `TELEGRAM_WEBHOOK_PATH` / `RELAY_HOST` / `RELAY_PORT` having no equivalent here is not
a gap to close for v1.0: the wider finding at §6 already established that the webhook runner is
not on the v1.0 path, which uses the long-poll runner instead.

The names that do matter for v1.0 are the first four rows. If the values in this
repository's env template were exported as-is into the other repository's process, the
long-poll runner would not find a bot token, would not know whether it is live, and would
refuse all traffic for want of an
operator whitelist (`auth.py` raises rather than defaulting open on a missing whitelist). This
is a real deployment hazard if the two repositories are ever wired together by copying one
environment file into the other verbatim. It is not evidence that either contract is wrong on
its own terms: each was audited clean against its own codebase, independently, in this
session and in prior sections of this file.

No task ID is assigned. Following the precedent at §7, this is logged as a tracked finding
about the other repository's naming, not an in-repo code defect, and no task-tracker entry
has been added.

## 13. Current deployment decision, verified 2026-08-15

The current public main revision remains `071e54c`.
Read-only GitHub inspection confirmed the following facts:

- No Dockerfile was found in the repository search.
- `NIZAM__system/relay/webhook.py` uses the standard-library HTTP server and says the production wrapper is still a future FastAPI and uvicorn step.
- `NIZAM__system/relay/auth.py` provides constant-time secret comparison and an allowlist.
- `NIZAM__system/relay/dedup.py` stores only `update_id`, so it is not namespaced by bot identity.
- The repository has no verified production life-agent image that can be deployed under Contract 12.

Decision: do not start the life-agent service with a placeholder image. The finance agent, signal bus,
and scheduler remain the active Phase 1 deployment. The life-agent service requires an owner-reviewed
cross-repository change, a production image, a health probe, the environment-name mapping, and tests
for the scheduler and signal-bus boundaries before it can be added to the live stack.

This inspection used read-only GitHub access. The other repository was not modified, committed, or
pushed. No credential, host particular, or messaging identifier was recorded here.

Aside, not part of this finding: §10 of `ops/INTEROP_CONTRACT.md` states the other repository
"has not been cloned, fetched, read, modified or pushed by any session on this side." This
file's own existence, and this section in particular, is now evidence against that sentence.
Noted here only; no correction has been made to that document as part of this task.
