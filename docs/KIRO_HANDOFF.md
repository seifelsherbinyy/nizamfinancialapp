...
| Gate | `npm run verify:all -- --all` must print `19 of 19 executed checks passed` |

First action: run the gate to prove the baseline. If it is not 19/19, stop and report.
...
- 333 tests / 37 files. Gate green at 19/19.
...
spec → implement → npm run typecheck → npm run lint → npm test
     → npm run verify:all -- --all          (must stay 19/19)
     → tick tasks.md
