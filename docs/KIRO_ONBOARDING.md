...
npm run verify:all -- --all  # MUST print "21 of 21 executed checks passed" before you change anything
```

If the harness is not 21/21 on a clean checkout, stop and report - do not build on a broken base.
...
## 3. Non-negotiable invariants (the harness enforces these - keep it at 21/21)
...
`npm run verify:all -- --all` (21/21) -> commit -> push to `master`.
...
21/21 now, and the suite has grown well past that figure):**
...
harness, 333 tests. That is a dated record; the live gate figure is the 21/21 stated in sections 1,
