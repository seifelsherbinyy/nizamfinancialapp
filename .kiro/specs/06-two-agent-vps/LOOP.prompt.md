...
5. GATE: `npm run verify:all -- --all` must print `HARNESS PASSED`, 20 of 20.
...
STOP only when all four hold in one cycle: (T1) no `[ ]`/`[-]` above "Waiting on user input"; (T2) 20/20 `HARNESS PASSED`; (T3) `node scripts/loop/verify-ledger.mjs` exits 0; (T4) `ops/GATE_REGISTER.md` complete plus a final report naming exactly ONE next human action. Three of four is red.
