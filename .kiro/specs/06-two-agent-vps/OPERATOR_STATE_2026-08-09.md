...
| **F12** | The firewall advice and the compose port bindings do not agree, and nothing asserts that they should. `ops/docker-compose.yml` publishes exactly one host port, `<TLS_PORT>` on `caddy`, and no other service has a `ports:` key. The G1 correction says to open 80 for the certificate challenge. Opening 80 in the host firewall reaches nothing if compose never binds it, so an HTTP-01 challenge fails while the firewall looks correct. Either publish 80 as a second port on `caddy`, or rely on TLS-ALPN-01 on the TLS port alone and record in the register that 80 is then not required. Blocks nothing in phase 1, because long-poll publishes no port at all. | `KIRO_SHIP_LIVE.prompt.md` section 2; `ops/docker-compose.yml` line 112 |
